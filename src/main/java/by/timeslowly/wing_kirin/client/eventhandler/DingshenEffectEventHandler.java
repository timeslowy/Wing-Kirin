...
/**
 * 定身效果客户端事件处理（自 1.21.1 NeoForge 分支移植）。
 * <p>
 * 1.20.1 适配：
 * <ul>
 *   <li>NeoForge RenderGuiLayerEvent + VanillaGuiLayers.CAMERA_OVERLAYS → Forge RenderGuiEvent.Post
 *       （1.20.1 无相机覆盖层，退而在整个 GUI 之后绘制金色遮罩，视觉层级略高于原版）；</li>
 *   <li>快捷栏蒙版 → RenderGuiOverlayEvent.Post + VanillaGuiOverlay.HOTBAR；</li>
 *   <li>InputEvent.InteractionKeyMappingTriggered 同名存在，逻辑不变。</li>
 * </ul>
 */
...
public class DingshenEffectEventHandler {
    ...
    // 定身金色遮罩渲染（1.20.1：在整个 GUI 之后绘制；1.21.1 为 CAMERA_OVERLAYS 层之后）
    @SubscribeEvent
    public static void onRenderDingShenVignette(RenderGuiEvent.@NotNull Post event) {
        Player player = Minecraft.getInstance().player;
        if (player == null) return;
        if (Minecraft.getInstance().options.hideGui) return;
        if (!player.hasEffect(WKEffects.DING_SHEN.get())) return;
    ...
    public static void onPlayerInteract(InputEvent.@NotNull InteractionKeyMappingTriggered event) {
        ...
        // 3. 判断是否拥有效果且配置启用禁用交互
        if (ConfigSyncHandler.dingShenDisableInteraction() && player.hasEffect(WKEffects.DING_SHEN.get())) {
            // 提示
    ...
    // 显示快捷栏禁用视觉效果
    @SubscribeEvent
    public static void onRenderHotbar(RenderGuiOverlayEvent.@NotNull Post event) {
