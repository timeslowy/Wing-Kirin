 ...
 * NeoForge 的便捷 API 会附带 {@code KnownPack} 声明，客户端于是只用本地资源解析这些注册表元素，
 * 而客户端本地的加载顺序与 {@code mod_data} 相反，覆盖会静默失效（无任何日志）。
 * 不声明 {@code KnownPack} 即可让服务端始终下发覆盖后的数据。
 * <p>
...
public class PackFinders {
    ...
    public static void addOverridingDataPack(
        ...
        PackLocationInfo locationInfo = new PackLocationInfo(
                "mod/" + ResourceLocation.fromNamespaceAndPath(Wing_kirin.MOD_ID, path),
    ...
    public static void addSimplePack(
        ...
        event.addPackFinders(
                ResourceLocation.fromNamespaceAndPath(Wing_kirin.MOD_ID, path),
                type,
                name,
                PackSource.FEATURE,
                alwaysActive,
    ...
    private static IModInfo modInfo() {
        var container = ModList.get().getModContainerById(Wing_kirin.MOD_ID).orElse(null);
