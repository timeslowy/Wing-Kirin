 ...
 * 伤害反弹结算（服务端）。
 * <p>
 * 玩家受到伤害后，读取 {@link DamageReflections} 数据中由
 * wing_kirin:damage_reflection 实体效果写入的参数，将伤害按比例反弹给玩家周围范围内的敌对实体
 * （敌对判定复用 DS 的 {@link TargetingMode#ENEMIES}，含 PvP 配置与队伍逻辑）。
 * <p>
 * 反伤基数取该伤害序列的<b>原始伤害</b>（护甲/抗性减免前）：
 * 即使抗性 V 等 100% 减免导致实际扣血为 0，仍按原始伤害 × 比例反伤。
 * <p>
 * 事件移植说明：1.21.1 用 NeoForge 的 {@code LivingDamageEvent.Post}（伤害落地后）+
 * {@code getOriginalDamage()}（减免前数值）；1.20.1 Forge 无该拆分，
 * 改用 {@link LivingHurtEvent}——它在护甲减免之前、所有取消/无敌/格挡判定之后触发，
 * {@code getAmount()} 即减免前原始伤害，语义等价。
 * <p>
 * 效果生命周期：数据条目带有效期（{@link DamageReflections#REFLECTION_TTL_TICKS}），
...
public class DamageReflectionEventHandler {
    ...
    @SubscribeEvent
    public static void onLivingDamage(final @NotNull LivingHurtEvent event) {
        ...
        if (source.is(WKDamageTypes.COUNTER_SHOCK)) {
            return;
        }

        // 本事件触发于护甲/抗性减免之前，getAmount() 即本段伤害序列的原始伤害。
    ...
    private static void reflect(final ServerPlayer player, final DamageSource original, final float amount, final DamageReflections.@NotNull Entry entry) {
        ...
        try {
            ...
            // damage_type 是纯数据包注册表、静态注册表集合中不存在该注册表，
            // 直接传入 ResourceKey 无法经网络编码，会导致 ClientboundDamageEventPacket 抛异常踢出客户端。
            Holder<DamageType> damageType = entry.useSameDamageType()
                    ? original.typeHolder()
                    : player.level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(WKDamageTypes.COUNTER_SHOCK);
