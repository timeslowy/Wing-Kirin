...
# 获得《仁者无敌》技能（未拥有时）
execute unless score @s wk.ability_add.has_ability matches 1 run dragon-ability add @s wing_kirin:invincible_benevolence

# 动作栏显示获得信息（未拥有时）
execute unless score @s wk.ability_add.has_ability matches 1 run title @s actionbar [{"translate": "actionbar.wing_kirin.upgrade.describiton_0"}, {"translate": "actionbar.wing_kirin.upgrade.invincible_benevolence.describiton_1","color": "#fbdc92"}]
