...
# 获得《唯快不破》技能（未拥有时）
execute unless score @s wk.ability_add.has_ability matches 1 run dragon-ability add @s wing_kirin:unstoppable_speed

# 动作栏显示获得信息（未拥有时）
execute unless score @s wk.ability_add.has_ability matches 1 run title @s actionbar [{"translate": "actionbar.wing_kirin.upgrade.describiton_0"}, {"translate": "actionbar.wing_kirin.upgrade.unstoppable_speed.describiton_1","color": "#fbdc92"}]
